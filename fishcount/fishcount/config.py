...
class AppConfig(BaseModel):
    ...
    `batch_size` left unset means "pick one for the device in use"; see
    fishcount.detector.default_batch_size for the measured values.

    Two confidence numbers, and only two. `conf` is the recording floor: every
    detection above it is written to the journal and detections.csv, so the
