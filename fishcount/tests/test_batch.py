...
def test_end_to_end_records_detections_and_blur(tmp_path: Path) -> None:
    ...
    for entry in entries.values():
        assert isinstance(entry["blur"], float)  # blur recorded for every readable frame

    # annotated copies exist only for frames with detections
    assert (out / "annotated" / "a.jpg").is_file()
    assert (out / "annotated" / "sub" / "b.png").is_file()
